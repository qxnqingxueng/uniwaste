...
class ChatListScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: const Text("Messages"), centerTitle: true, automaticallyImplyLeading: false),
      body: StreamBuilder<QuerySnapshot>(
        stream: chatService.getUserChats(user.userId),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) return const Center(child: CircularProgressIndicator());
          final docs = snapshot.data?.docs ?? [];
          if (docs.isEmpty) return const Center(child: Text("No chats yet. Claim food to start!"));
          ...
          return ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              names.forEach((key, val) { if (key != user.userId) otherName = val; });
